...
- 页面状态路由：识别当前页面文字后跳到命名标签，自动纠正跑错阶段
- 内置托管看门狗：画面或关键文字长时间不变时自动重启流程或指定程序
- 恢复记录：卡住时自动保存截图和恢复日志
...
1. 下载最新的 `FlowClickStudio-v0.3.0-windows-x64.zip`。
2. 右键选择“全部解压缩”。
...
先添加“流程标签”，例如“开箱”“领奖”“再来”，再添加“识别页面并跳转”。每行规则使用：
...
程序识别到页面文字后会直接进入对应标签；识别不到则继续下一步。跳转可以退出当前的小循环，但不能从循环外跳入尚未开始的循环。

### 托管看门狗

把“启动托管看门狗”放在流程开头。它会同时检查画面变化和关键页面文字：

- 达到设定的卡住时间后，可只重启流程；
- 也可以关闭指定进程、重新启动 EXE，等待游戏加载后再从流程开头运行；
- 卡住截图与日志保存在 `%LOCALAPPDATA%\FlowClickStudio\recovery`；
- `最多自动恢复次数` 填 `0` 表示不限次数，建议先填 `10` 测试稳定性。

游戏通过启动器运行时，“程序 EXE 路径”应选择实际可启动游戏的 EXE；“进程名”填写需要被关闭的游戏进程名。请先用较短流程测试一次自动恢复，再进行长时间托管。
...
4. 输出 `output\FlowClickStudio-v0.3.0-windows-x64.zip`。

### 使用 GitHub Actions

将代码推送到 GitHub 的 `main` 分支后，打开仓库的 **Actions → Build Windows EXE → Run workflow**。完成后从页面底部的 **Artifacts** 下载 `FlowClickStudio-v0.3.0-windows-x64`。
